...
export default function EditInvoicePage() {
            ...
            try {
                ...
                if (res.success && res.data) {
                    ...
                    setForm({
                        ...
                        destination: data.destination || 'DAMMAM, SAUDI ARABIA',
                        notes: data.notes || '',
                        invoiceDate: data.invoiceDate ? new Date(data.invoiceDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
                        items: data.items && data.items.length > 0 ? data.items : [{ description: '', qty: 1, unitPriceKrw: 0, unitPriceUsd: 0, unitPriceSar: 0 }],
                ...
                alert(isRTL ? 'خطأ في تحميل بيانات الفاتورة' : 'Error loading invoice data');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                alert(isRTL ? 'فشل حفظ الفاتورة' : 'Failed to update invoice');
            }
        } catch (error: any) {
    ...
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center font-mono text-white/50 tracking-widest">LOADING...</div>;
    }
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-white/40 text-[10px] font-mono mt-0.5 tracking-[0.2em] uppercase italic">
                            {isRTL ? 'تسجيل رسوم وأسعار الشراء وإضافة النقل' : 'Modify parameters for HM CAR documentation'}
                        </p>
                    </div>
                </div>
                    ...
                    <div className="bg-white/[0.03] border border-white/[0.05] p-8 rounded-3xl backdrop-blur-md relative overflow-hidden group">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-black uppercase tracking-widest text-white/30 ml-1">Invoice Number</label>
                                <input 
